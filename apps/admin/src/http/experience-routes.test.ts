...
/**
 * Phase 4 T4.10 — regression guard for the new Experience read routes (list/get). Same
...
describe("experience routes — read side (Phase 4 T4.10)", () => {
  it("list/get return flat DTOs with no aggregate internals", async () => {
    const admin = buildAdmin();
    const routes = experienceRoutes(admin);

    const created = unwrap<{ experienceId: string }>(
      await call(
        byPathAndMethod(routes, "POST", "/experiences"),
        {},
        {},
        { name: "Homepage", experienceType: "storefront" },
      ),
      "create experience",
    );

    const listResponse = await call(byPathAndMethod(routes, "GET", "/experiences"), {}, {});
    ...
    const getResponse = await call(
      byPathAndMethod(routes, "GET", "/experiences/:experienceId"),
