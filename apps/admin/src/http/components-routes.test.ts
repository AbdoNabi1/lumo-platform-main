...
/**
 * Phase 4 T4.9 — regression guard for the new Components read routes (list/get). Same technique
...
describe("components routes — read side (Phase 4 T4.9)", () => {
  it("list/get return flat DTOs with no aggregate internals", async () => {
    const admin = buildAdmin();
    const routes = componentsRoutes(admin);

    const created = unwrap<{ componentDefinitionId: string }>(
      await call(
        byPathAndMethod(routes, "POST", "/components"),
        {},
        {},
        {
          key: "hero",
    ...
    const listResponse = await call(byPathAndMethod(routes, "GET", "/components"), {}, {});
    ...
    const getResponse = await call(
      byPathAndMethod(routes, "GET", "/components/:componentDefinitionId"),
      { componentDefinitionId: created.componentDefinitionId },
      {},
    );
    ...
    for (const dto of [
      ...
      expect(dto).toMatchObject({
        id: created.componentDefinitionId,
        key: "hero",
        name: "Hero",
        status: "draft",
      });
    ...
    const notFound = await call(
      byPathAndMethod(routes, "GET", "/components/:componentDefinitionId"),
      { componentDefinitionId: "does-not-exist" },
      {},
    );
