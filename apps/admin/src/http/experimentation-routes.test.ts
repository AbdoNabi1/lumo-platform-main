...
/**
 * Phase 4 T4.17 — regression guard for the new Experimentation read routes (list/get). Same
...
describe("experimentation routes — read side (Phase 4 T4.17)", () => {
  it("list/get return flat DTOs with no aggregate internals", async () => {
    const admin = buildAdmin();
    const routes = experimentationRoutes(admin);

    const created = unwrap<{ experimentId: string }>(
      await call(
        byPathAndMethod(routes, "POST", "/experiments"),
        {},
        {},
        {
    ...
    const listResponse = await call(byPathAndMethod(routes, "GET", "/experiments"), {}, {});
    ...
    const getResponse = await call(
      byPathAndMethod(routes, "GET", "/experiments/:experimentId"),
