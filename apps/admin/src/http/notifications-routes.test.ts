...
/**
 * Phase 4 T4.12 — regression guard for the new Notifications read routes (list/get). Same
...
describe("notifications routes — read side (Phase 4 T4.12)", () => {
  it("list/get return flat DTOs with no aggregate internals", async () => {
    const admin = buildAdmin();
    const routes = notificationsRoutes(admin);

    const created = unwrap<{ notificationId: string }>(
      await call(byPathAndMethod(routes, "POST", "/notifications"), {}, {}, createBody()),
      "create notification",
    );

    const listResponse = await call(byPathAndMethod(routes, "GET", "/notifications"), {}, {});
    ...
    const getResponse = await call(
      byPathAndMethod(routes, "GET", "/notifications/:notificationId"),
