...
import { AlertTriangleIcon, BellIcon, LockIcon, PlusIcon } from "lucide-react";
...
import { fetchNotificationsPage } from "@/lib/api/notifications";
...
/**
 * The Notifications list screen (T5.11a). No frontend existed for the Notifications domain before
 ...
 * `GET /notifications`. Never falls back to demo data on failure, same discipline as every prior
 * Phase 5 list screen.
 */
    ...
    <AppShell t={t} locale={locale} activeNavId="notifications" user={user}>
      <div className="mx-auto flex w-full max-w-[1600px] flex-col gap-6">
        <header className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h1 className="text-4xl font-semibold tracking-tight">{t.notificationsPage.title}</h1>
            <p className="text-md text-muted-foreground mt-1">{t.notificationsPage.subtitle}</p>
          </div>
          <Button asChild>
        ...
        <Suspense key={params.after ?? ""} fallback={<NotificationsTableSkeleton />}>
          <NotificationsResults first={PAGE_SIZE} after={params.after} t={t} />
        </Suspense>
      </div>
    </AppShell>
  );
...
async function NotificationsResults({
  first,
  after,
  t,
}: {
  ...
  const result = await fetchNotificationsPage({ first, after });

  if (result.outcome === "unauthorized") {
    return (
      <StatePanel
        icon={<LockIcon aria-hidden="true" className="size-5" />}
        message={t.notificationsPage.unauthorized}
      />
    );
  }
  if (result.outcome === "error") {
    return (
      <StatePanel
        ...
        message={t.notificationsPage.error}
      />
    );
  }
  if (result.items.length === 0) {
    return (
      <StatePanel
        ...
        message={t.notificationsPage.empty}
      />
    );
  }
  ...
  return (
    <Card>
      <CardContent className="p-0 sm:p-0">
        <NotificationsTable notifications={result.items} t={t} />
      </CardContent>
