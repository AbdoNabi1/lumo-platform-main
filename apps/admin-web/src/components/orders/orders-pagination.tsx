...
/**
 * Cursor-only pagination (the backend `GET /orders` has no `before`/offset support — only a
 * forward `after` cursor, `services/orders/src/domain/order-repository.ts`). "Previous" is
 * implemented client-side as a back-stack of prior cursors kept in the `stack` URL param (each
 * entry is the `after` value that produced that earlier page, `""` meaning "the first page") —
 * not a backend capability that doesn't exist. This is standard practice for a forward-only
 * cursor API and keeps the URL shareable/back-button-safe.
 */
