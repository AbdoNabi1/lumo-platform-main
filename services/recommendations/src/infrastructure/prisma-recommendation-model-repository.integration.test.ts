...
describe.runIf(Boolean(databaseUrl))("PrismaRecommendationModelRepository (integration)", () => {
  ...
  function wire(tenantId: string) {
    ...
    const repository = new PrismaRecommendationModelRepository({ prisma, tenantId, outbox, context });
    const unitOfWork = new PrismaUnitOfWork(prisma);
    return {
      prisma,
      repository,
      save: (model: RecommendationModel) => unitOfWork.run((tx) => repository.save(model, tx)),
    };
  }
