...
/**
 * Phase 4 T4.16 — real PostgreSQL coverage for the new `list` read, following the same reference
 * pattern as `services/reviews/src/infrastructure/prisma-review-repository.integration.test.ts`.
 *
 *   DATABASE_URL_TEST=postgresql://lumo:lumo@localhost:5432/lumo_test pnpm --filter @platform/feature-flags-service test
 */
const databaseUrl = process.env["DATABASE_URL_TEST"];

describe.runIf(Boolean(databaseUrl))("PrismaFeatureFlagRepository (integration)", () => {
  ...
  it("list filters by tenantId and paginates", async () => {
    const tenantId = `tenant-itest-feature-flags-${crypto.randomUUID()}`;
    const other = `tenant-itest-feature-flags-other-${crypto.randomUUID()}`;
    const { prisma, repository, save } = wire(tenantId);
    const { repository: otherRepository, save: saveOther } = wire(other);

    for (let i = 0; i < 3; i += 1) {
      await save(FeatureFlag.create(UniqueEntityId.from(ids.generate()), `flag-${i}`, `Flag ${i}`));
    }
    await saveOther(FeatureFlag.create(UniqueEntityId.from(ids.generate()), "flag-x", "Flag X"));
