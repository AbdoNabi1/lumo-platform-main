...
describe("DeleteSegment", () => {
  it("removes the definition from future evaluation and publishes SegmentDeleted", async () => {
    ...
    const result = await del.execute({ id: "high_value", expectedVersion: 1 });
    expect(result.ok).toBe(true);
    if (!result.ok) throw new Error("unreachable");
    expect(result.value.deleted).toBe(true);

    expect(await definitions.getById("high_value")).toBeNull();
  ...
  it("rejects a stale expectedVersion with ConcurrencyError and never deletes", async () => {
    ...
    await expect(del.execute({ id: "high_value", expectedVersion: 99 })).rejects.toBeInstanceOf(
      ConcurrencyError,
    );
  ...
  it("rejects deleting a segment that does not exist", async () => {
    const { del } = wire();
    const result = await del.execute({ id: "does_not_exist", expectedVersion: 1 });
  ...
  it("never cascades into existing SegmentMembership/SegmentHistory rows", async () => {
    ...
    await history.append(toSnapshot(membership, "entered", "t0"), undefined);

    await del.execute({ id: "high_value", expectedVersion: 1 });

    expect(await segments.getCurrent(identifier, "high_value")).not.toBeNull();
    expect(await history.listFor(identifier, "high_value")).toHaveLength(1);
  });
});
