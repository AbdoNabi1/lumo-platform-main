...
describe("composable policy language / security registry / compliance (end to end)", () => {
  it("evaluates fragment-composed policy, versions registries, and reports compliance", async () => {
    ...
    await app.security.definePolicy({ key: "p1", name: "Composed", mode: "custom" });
    ...
    expect(
      body<{ effect: string }>(
        await app.security.simulatePolicy({ key: "p1", context: { risk: 90 } }),
      ).effect,
    ).toBe("block");

    // ── §6 versioned security registry ──
    await app.security.registerAuthMethod({ kind: "password", displayName: "Password" });
    await app.security.registerMfaMethod({ kind: "totp", displayName: "Authenticator" });
    ...
    expect(
      (await app.security.evaluateCompliance({ framework: "gdpr", tenantRef: "t1" })).status,
