...
export class GetReportDefinition
  implements UseCase<ReportDefinitionIdInput, ReportDefinition, DomainError>
{
  ...
  async execute(
    input: ReportDefinitionIdInput,
  ): Promise<Result<ReportDefinition, DomainError>> {
    const definition = await this.deps.reportDefinitions.findById(input.reportDefinitionId);
