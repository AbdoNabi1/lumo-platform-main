...
describe("RecordPromotionUsage error handling (Phase A.17)", () => {
  it("returns a 409 domain-error response, not an unhandled rejection, when usage-limit crossing hits an illegal transition", async () => {
    ...
    await app.promotions.advance({ promotionId: id, toStatus: "active" });
    await app.promotions.advance({ promotionId: id, toStatus: "paused" });
    ...
    const response = await app.promotions.recordUsage({ promotionId: id });
  ...
  it("leaves the promotion's usageCount/status unchanged when the transition is rejected", async () => {
    ...
    await app.promotions.recordUsage({ promotionId: id });

    const secondUsage = await app.promotions.recordUsage({ promotionId: id });
