...
/** Persistence port for {@link InventoryItem}. Implemented in infrastructure. The optional `tx` scopes the call to the caller's transaction (ADR-0003). */
export interface InventoryItemRepository {
  save(item: InventoryItem, tx?: unknown): Promise<void>;
  findById(id: string, tx?: unknown): Promise<InventoryItem | null>;
