...
export class ListIndexes implements UseCase<CursorPage, Paginated<SearchIndex>, DomainError> {
  ...
  async execute(input: CursorPage): Promise<Result<Paginated<SearchIndex>, DomainError>> {
    return ok(await this.deps.indexes.list(input));
  }
}
