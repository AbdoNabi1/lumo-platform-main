...
describe("licensing (end to end)", () => {
  it("creates a plan, publishes a version, and starts a subscription pinned to it", async () => {
    const app = wire();
    const plan = await app.licensing.createPlan({ key: "growth", name: "Growth", tier: "growth" });
    expect(plan.status).toBe(201);
    const planId = (plan.body as { id: string }).id;

    const draft = await app.licensing.createPlanDraft({ planId, spec: spec() });
    expect(draft.status).toBe(201);
    const planVersionId = (draft.body as { planVersionId: string }).planVersionId;

    const published = await app.licensing.publishPlanVersion({ planId, planVersionId });
  ...
  it("rejects creating a duplicate plan key (409)", async () => {
    const app = wire();
    await app.licensing.createPlan({ key: "growth", name: "Growth", tier: "growth" });
  ...
  it("rejects publishing an unknown plan version (404)", async () => {
    const app = wire();
    const plan = await app.licensing.createPlan({ key: "pro", name: "Pro", tier: "pro" });
    const planId = (plan.body as { id: string }).id;
    const response = await app.licensing.publishPlanVersion({ planId, planVersionId: "missing" });
