...
export interface ListMediaAssetsInput extends CursorPage {
...
export class ListMediaAssets implements UseCase<
...
> {
  ...
  async execute(input: ListMediaAssetsInput): Promise<Result<Paginated<MediaAsset>, DomainError>> {
    return ok(await this.deps.mediaAssets.list(input, input.tenantId));
  }
}
