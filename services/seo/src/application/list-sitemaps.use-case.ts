...
export class ListSitemaps implements UseCase<CursorPage, Paginated<Sitemap>, DomainError> {
  ...
  async execute(input: CursorPage): Promise<Result<Paginated<Sitemap>, DomainError>> {
    return ok(await this.deps.sitemaps.list(input));
  }
}
