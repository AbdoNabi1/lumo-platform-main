...
describe("pricing (end to end)", () => {
  it("creates + activates a price list, then creates + changes a price emitting price.changed", async () => {
    ...
    const list = await app.priceLists.create({ tenantId: TENANT, name: "Retail", currency: "USD" });
    expect(list.status).toBe(201);
    const listId = (list.body as { id: string }).id;
    expect((await app.priceLists.activate({ tenantId: TENANT, priceListId: listId })).status).toBe(
      200,
    );
    ...
    const changed = await app.prices.change({
      tenantId: TENANT,
      priceId,
      amountMinor: 1799,
      currency: "USD",
  ...
  it("rejects an invalid currency (422)", async () => {
    const app = wire();
    const response = await app.priceLists.create({
      tenantId: TENANT,
      name: "Bad",
      currency: "dollars",
    });
  ...
  it("creates a rich price (compare-at/cost/effective window/tax class) and publishes it", async () => {
    const app = wire();
    const list = await app.priceLists.create({
      tenantId: TENANT,
      name: "Retail 2",
      currency: "USD",
    });
    ...
    const taxClass = await app.registry.createTaxClass({
      tenantId: TENANT,
      code: "STANDARD",
      name: "Standard rate",
    });
    ...
    const published = await app.prices.publish({ tenantId: TENANT, priceId });
    expect(published.status).toBe(200);
    expect((published.body as { status: string }).status).toBe("published");

    const doublePublish = await app.prices.publish({ tenantId: TENANT, priceId });
    expect(doublePublish.status).toBe(409);
  });

  it("rejects a duplicate tax class code (dedupe, 409)", async () => {
    const app = wire();
    await app.registry.createTaxClass({ tenantId: TENANT, code: "REDUCED", name: "Reduced rate" });
    const duplicate = await app.registry.createTaxClass({
      tenantId: TENANT,
      code: "REDUCED",
      name: "Reduced (dup)",
    });
  ...
  it("rejects an invalid tax class (empty code, 422)", async () => {
    const app = wire();
    const response = await app.registry.createTaxClass({
      tenantId: TENANT,
      code: "",
      name: "Nameless",
    });
  ...
  it("lists prices with cursor pagination", async () => {
    ...
    const page = await app.prices.list({ tenantId: TENANT, first: 2 });
