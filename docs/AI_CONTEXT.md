...
1. The **Lumo Design System** — [`docs/ui/`](ui/README.md) is the visual source of truth and the platform's only design system; build from `@platform/ui` on `@platform/design` tokens, never hard-code visual values, never start a parallel system.
