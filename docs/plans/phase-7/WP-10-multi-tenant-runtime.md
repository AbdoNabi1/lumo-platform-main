...
**Status 2026-09-26 (definition-of-done pass): WP-10 is NOT done, and `TENANT_MODE=multi` is NOT safe to enable.**
The one thing that makes it unsafe is **G-75**: the worker's tracking event-record store addresses rows by
`eventId` alone although the key is `(tenantId, eventId)` and clients supply the id, so history for one
tenant's event can land on another tenant's record. It was found by this pass and deliberately not fixed in
it (held as an `it.fails`). Beyond that, isolation rests on application code alone — RLS is inert (the
connecting role has `BYPASSRLS`) — and only the API path was proven end to end. Full evidence:
[`WP-10-guard-verification.md`](WP-10-guard-verification.md) (157 mutations), ADR-0014 Amendment 9, D-069.

- [ ] `TENANT_MODE=multi` boots, and two tenants serve correct, isolated data through the same process.
      _Boots, and one Fastify instance serves two tenants for reads, writes, authorization decisions and
      shared caches (`apps/runtime/src/multi-tenant-boot.e2e.test.ts`, 22 tests). Not reached: Postgres (the
      in-memory composition is what boots), the worker's consumers (covered by their own suites, not booted),
      and G-75 falsifies the isolation claim on the tracking path._
- [x] Every test in T10.5 passes, and each one fails if you deliberately remove the guard it covers.
      _Verified by mutation: 145 first-pass mutations, 31 survived (26 guards + 5 Prisma samples), every one
      reported and then covered — see the survivors table. The largest: the Prisma `where: { tenantId }` of
      orders, catalog and inventory had no test that could fail; `scripts/dev/check-prisma-tenant-where.mjs`
      now guards all of them uniformly._
- [x] A request with no resolvable tenant is rejected, never defaulted. _Absent/empty/blank/non-string/
      malformed/unknown/suspended/mismatched, for session claim, header and event envelope; two real bugs
      found and fixed (blank claims resolved; Kratos traits could override the claim)._
- [x] The ADR is written, registered (D-053), and matches what the code does. _ADR-0014 Amendment 9 and
      inline `[As built]` notes; Decision 2 is recorded as NOT implemented as written._
- [ ] G-53 closed. _Open: closes when G-75 is fixed._ G-23/G-32/G-38 updated with what actually changed: [x].
- [ ] Repo-wide gates green, plus `pnpm arch`. _See the commit that adds this line._
