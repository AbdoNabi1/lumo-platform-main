...
**T10.6 (2026-09-26, D-068): tenant lifecycle is real.** Suspension and cancellation are enforced where the tenant is resolved (a suspended tenant keeps read access and invoice payment, nothing else; ≤10 s cross-instance staleness), provisioning gives a new tenant a working security baseline idempotently and reports itself incomplete when it is, and the platform tenant cannot be suspended or cancelled. The worker guard no longer refuses `TENANT_MODE=multi` for `bootstrapSecurity` (no boot pin remains) — `TENANT_MODE=multi` is **still not enabled**. G-32 and G-25 are **narrowed, not closed**: nothing is erased.
...
The Keto tuples are now tenant-qualified in code: all four writers dual-write `tenant/<tenantId>/<permission>` beside the bare tuple (the consumer takes the tenant from the event envelope and fails closed without one — D-056), reads are switched (`KetoAccessControl.objectFor`, and Security's ReBAC check), and three operator scripts — backfill, count gate, delete-bare — exist and are unit-tested against a fake. **G-70 is not closed:** the live Ory Network tuples are still bare, and nothing in this repo has been run against Ory. **`TENANT_MODE=multi` must not be enabled until the runbook (`docs/operations/KETO_TENANT_MIGRATION.md`) has been run** — backfill, then the count gate, then delete-bare — and the post-delete gate is green; G-64 (the worker refuses multi) and G-68's legacy-storage-key count also stand. Until the backfill runs, a developer running the runtime against the shared Ory project is denied every permission. Nothing enables multi anywhere.
