...
import { createPrismaClient, PrismaOutboxStore, PrismaUnitOfWork } from "@platform/db";
...
describe.runIf(Boolean(databaseUrl))("Prisma Identity access repositories (integration)", () => {
  ...
  const openClients: ReturnType<typeof createPrismaClient>[] = [];
  ...
  describe("PrismaUserRepository", () => {
    ...
    it("writes the outbox row (user.created) in the SAME transaction as the aggregate", async () => {
      const { users, unitOfWork, outboxStore } = wire();
      const t = tenant();
      const id = UniqueEntityId.from(ids.generate());
      const user = User.create(id, t, email(), "Alice", ids.generate(), clock.now());

      await unitOfWork.run((tx) => users.save(user, tx));
      const pending = await outboxStore.fetchPending(500);
      expect(pending.some((e) => e.key === id.toString() && e.topic.includes("user"))).toBe(true);
    });
  });
  ...
  describe("PrismaMembershipRepository", () => {
    ...
    it("writes the outbox row (membership.role_changed) in the SAME transaction", async () => {
      const { memberships, unitOfWork, outboxStore } = wire();
      const t = tenant();
      const id = UniqueEntityId.from(ids.generate());
      const membership = Membership.create(
        id,
      ...
      const pending = await outboxStore.fetchPending(500);
      expect(pending.some((e) => e.key === id.toString() && e.topic.includes("role_changed"))).toBe(
        true,
      );
    });
  });
