...
## 6. Addendum — Post-release reproducibility repair

**Date:** 2026-07-27
**Commit:** `ecb4a05461d83b5226a0f36111e19d7353abb165` — `fix(lockfile): restore Sprint A0
reproducibility` (parent: `830138a`, the Sprint A0 baseline-record commit; grandparent:
`2dc8e0b`, the Sprint A0 implementation commit).

**Reason:** as originally committed, `2dc8e0b`'s `pnpm-lock.yaml` did not include importer
entries for the `@platform/repository` (`workspace:*`) dependency that both
`packages/messaging/package.json` and `packages/kafka/package.json` already declared in that same
commit. `pnpm install --frozen-lockfile` — the install mode a clean clone or CI must use — failed
with `ERR_PNPM_OUTDATED_LOCKFILE`. Sprint A0, as shipped, was not reproducible from a clean
checkout.

**Proof that only the lockfile importer entries changed:** the repair commit's diff is exactly
6 lines across 2 hunks, both confined to the `importers:` section of `pnpm-lock.yaml`:
...
`git show --stat ecb4a05`: `pnpm-lock.yaml | 6 ++++++`, 1 file changed. No `package.json`, source
file, governance file, Husky config, lint-staged config, architecture doc, or dependency version
changed. No `packages:`/`snapshots:` entry, integrity hash, or `lockfileVersion` changed — the
fix added two missing links to an already-resolved workspace package, nothing new was resolved.

Full analysis and landing evidence: `SPRINT_A0_LOCKFILE_REPAIR_ANALYSIS.md`,
`SPRINT_A0_REPRODUCIBILITY_REPAIR_REPORT.md`, `SPRINT_A0_REPAIR_LANDING_REPORT.md`.

**Sprint A0's frozen implementation commit (`2dc8e0b`) and baseline-record commit (`830138a`)
were not amended or rewritten** — this is a new, superseding commit on top of them, per the
project's standing no-history-rewriting rule.
